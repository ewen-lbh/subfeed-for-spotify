...
export type Tokens = { access: string; refresh: string; expiresIn: number; grantedScopes: string[]; verifier?: string; }

export async function makeAuthorizationRequestURL(codeVerifier: string, scopes: string[]): Promise<string> {
	return ("https://accounts.spotify.com/authorize"
		+ "?client_id=" + encodeURIComponent(CLIENT_ID)
		+ "&response_type=code"
		+ "&redirect_uri=" + encodeURIComponent(REDIRECT_URI)
		+ "&code_challenge_method=S256"
		+ "&code_challenge=" + encodeURIComponent(await codeChallengeFromVerifier(codeVerifier))
		/*TODO + "&state=" + */
		+ "&scope=" + encodeURIComponent(scopes.join(" "))
	)
}

export async function getTokens(authorizationCode: string, codeVerifier: string): Promise<Tokens> {
...
export function readTokensFromLocalStorage(): Tokens {
	...
	if (localTokensString !== null) {
		try {
			...
			console.warn(`Couldn't load from localStorage: value was not correct JSON`)
